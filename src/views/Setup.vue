...
<template>
    <div id="setup" :class="{ 'fix': fix }" v-italic>
        ...
        <div id="layouts">
            <div id="classic-layout" class="layout" @click="select('classic')"
                :class="{ selected: settings.mode === 'classic' }">
                <LLogin id="classic" :immutable="true" :compact="false" :preview="true" />
            </div>

            <div id="compact-layout" class="layout" @click="select('compact')"
                :class="{ selected: settings.mode === 'compact' }">
                <LLogin id="compact" :immutable="true" :compact="true" :preview="true" />
            </div>
        </div>

        <div id="left-settings" class="settings">
            <div class="checkbox-line">
                <LCheckbox v-model="settings.disableSplash" /><label>{{ texts.disableSplash }}</label>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.disableSplashText" /><label>{{ texts.disableSplashText }}</label>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.disableIntro" /><label>{{ texts.disableIntro }}</label>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.clock12" /><label>{{ texts.clock12 }}</label>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.disablePowerTexts" /><label>{{ texts.disablePowerTexts }}</label>
            </div>
        </div>

        <div id="right-settings" class="settings">
            <div class="disable-zoom">-<div class="checkbox-line">
                    <LCheckbox v-model="settings.disableZoom" /><label>{{ texts.disableZoom }}</label>
                </div>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.disableFade" /><label>{{ texts.disableFade }}</label>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.roundAvatar" /><label>{{ texts.roundAvatar }}</label>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.disableAvatar" /><label>{{ texts.disableAvatar }}</label>
            </div>
            <div class="checkbox-line">
                <LCheckbox v-model="settings.hideUsername" /><label>{{ texts.hideUsername }}</label>
            </div>
            <div class="checkbox-line"><router-link to="/blur" class="blur-settings">{{ texts.blurSettings }}</router-link>
            </div>
        </div>

        <LPowerButton id="back" type="back" />
        <LPowerButton id="theming" type="theming" />
    </div>
</template>
